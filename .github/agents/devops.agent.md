...
- usages
handoffs:
- label: Adjust backend for infra changes
  agent: backend-impl
  prompt: Update backend configuration and code to align with the DevOps/infra changes
    described above.
  send: false
- label: Adjust frontend for deployment
  agent: frontend-impl
  prompt: Update frontend configuration (envs, base URLs, feature flags) to match
    the deployment setup above.
  send: false
---
