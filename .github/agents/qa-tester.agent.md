---
name: qa-tester
description: Custom Copilot agent 'qa-tester' for Presence projects.
target: vscode
handoffs:
- label: Implement tests
  agent: tests-impl
  prompt: Implement or update automated tests (Jest and others if applicable) according
    to the test matrix above.
  send: false
- label: Fix implementation according to tests
  agent: dev-impl
  prompt: Adjust the implementation so that it satisfies the test matrix and expected
    behavior above, keeping code clean and maintainable.
  send: false
- label: Sync documentation
  agent: docs-architect
  prompt: Update the documentation so that it reflects the validated behavior and
    test cases above.
  send: false
---
